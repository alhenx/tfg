...
void CAutopilot::Update( CCommand& command )
{
	...
	valorBrujula = NDataManager::m_navData.HDGD;
	if(valorBrujula>180){
		valorBrujula-=360;
	}

	float resta = valorBrujula - brujula_old;

	if(abs(resta)<=margen_max){
	  if(abs(resta)>=margen_min){
	    brujula += resta;
	  }
	  brujula_old = valorBrujula;
	}
	Serial.print( F( "BRUJULA:" ) );
	Serial.print(brujula);
	Serial.println( ';' );
	/*
	if( command.Equals( "brujb" ) ){
		brujula=command.m_arguments[1];
	}

	if( command.Equals( "bruji" ) ){
		margen_min=command.m_arguments[1];
		Serial.print( F( "MARGENINF:" ) );
		Serial.print(margen_min);
		Serial.println( ';' );
	}

	if( command.Equals( "brujs" ) ){
		margen_max=command.m_arguments[1];
		Serial.print( F( "MARGENSUP:" ) );
		Serial.print(margen_max);
		Serial.println( ';' );
	}*/
